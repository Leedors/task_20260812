...
| 요구사항 | 구현 |
| --- | --- |
| 로그 기능 | 요청 로깅 미들웨어(상관관계 ID·상태코드·소요시간) + CQRS `LoggingBehavior` + 개인정보 차단(5.13) |
...
| (추가) CI | Ubuntu·Windows 양쪽에서 .NET 8 빌드 + 테스트 192개, 도커 이미지 기동 확인까지 자동화 |
| (추가) 컨테이너 실행 | 멀티스테이지 Dockerfile + compose. .NET SDK 없이도 결과 확인 가능 |
...
| --- | :---: | :---: | :---: | :---: |
| **.NET 8 SDK 8.0.423** (런타임 8.0.29 단독, 로컬) | ✅ 경고 0 | ✅ | ✅ | ✅ |
| **CI — Ubuntu** (.NET 8 SDK) | ✅ | ✅ | ✅ | ✅ 컨테이너 |
| **CI — Windows** (.NET 8 SDK) | ✅ | ✅ | ✅ | — |
| .NET 10 SDK preview (런타임 9.0 / 10.0, 로컬) | ✅ 경고 0 | ✅ | ⚠️ 아래 참고 | ✅ |

로컬 검증은 빈 디렉터리에 **저장소를 새로 clone 해서** 진행했습니다.
.NET 8 만 설치된 환경에서 192개 테스트가 모두 통과하고, 필수 4가지 입력·검색·수정·제외·헬스체크·Swagger 가
정상 동작하는 것까지 확인했습니다. CI 는 매 커밋마다 같은 확인을 반복합니다.
...
CI 러너에도 상위 런타임이 함께 설치돼 있어 이 현상이 그대로 재현됐습니다.
CI 는 "선언한 타겟에서 동작하는가"를 확인해야 하므로 테스트 단계에만
`DOTNET_ROLL_FORWARD=LatestPatch` 를 걸어 8.0 계열로 고정했습니다.
애플리케이션 실행 시의 롤포워드 동작은 그대로입니다.
